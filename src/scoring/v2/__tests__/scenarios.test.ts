...
//   composition 0.50, manufacturing 0.25, qpr 0.25
...
describe("Worthy Score v2 - scenari simbolo", () => {
  ...
  it("scenario 3 - lana merino Italy 200€ → worthy", () => {
    const input: WorthyScoreV2Input = {
  ...
  it("scenario 6 - cashmere Italy Made-in-Italy 100% 1200€ → worthy", () => {
    const input: WorthyScoreV2Input = {
      composition: [{ fiber: "cashmere", percentage: 100 }],
      price: 1200,
      category: { avgCompositionScore: 78, avgPrice: 100 },
      manufacturing: { productionCountry: "IT" },
      productCertifications: ["made_in_italy_100"],
    };
    const r = calculateWorthyScoreV2(input);
  ...
  it("scenario 7 - capo legacy (solo composition+price) → confidence 75", () => {
    const input: WorthyScoreV2Input = {
      composition: [
  ...
  it("scenario 8 - bonus Made-in-Italy 100% alza manufacturing di +8", () => {
    const base: WorthyScoreV2Input = {
      composition: [{ fiber: "cotton", percentage: 100 }],
      price: 50,
      category: { avgCompositionScore: 60, avgPrice: 50 },
      manufacturing: { productionCountry: "IT" },
    };
    const without = calculateWorthyScoreV2(base);
    const withCert = calculateWorthyScoreV2({
      ...base,
      productCertifications: ["made_in_italy_100"],
    });
    // Il bonus +8 sul manufacturing (peso 25%) si traduce in +2 sul finale.
    expect(withCert.score).toBeGreaterThan(without.score);
    expect(withCert.breakdown.lenses.manufacturing.score).toBe(
...
describe("Worthy Score v2 - graceful degradation", () => {
  it("solo composition+qpr (manufacturing null) - nessun crash, confidence 75", () => {
  ...
  it("confidence 100% solo se TUTTE le 3 lenti hanno dati", () => {
    const r = calculateWorthyScoreV2({
      composition: [{ fiber: "merino wool", percentage: 100 }],
      price: 200,
      category: { avgCompositionScore: 78, avgPrice: 100 },
...
describe("Worthy Score v2 - breakdown", () => {
  it("breakdown espone le 3 lenti con i pesi nuovi", () => {
    const r = calculateWorthyScoreV2({
      composition: [{ fiber: "cotton", percentage: 100 }],
      price: 30,
      category: { avgCompositionScore: 60, avgPrice: 25 },
